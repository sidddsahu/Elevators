...
const formatProjectData = (
  ...
  return {
    ...projectData,
    tags: parseArrayField(projectData.tags),
    features: parseArrayField(projectData.features),
    challenges: parseArrayField(projectData.challenges),
    images: parseArrayField(projectData.images)
  };
};
...
>(
  ...
  async (params = {}, { rejectWithValue }) => {
    try {
      ...
      const res = await axios.get<FetchProjectsResponse>(
        `${API_URL}?${queryParams}`
      );
...
>(
  ...
  async ({ id, projectData }, { rejectWithValue }) => {
    try {
      ...
      const res = await axios.put<SingleProjectResponse>(
        `${API_URL}/${id}`,
        formattedData
      );
...
>(
  ...
  async (params = {}, { rejectWithValue }) => {
    try {
      ...
      const res = await axios.get<SingleProjectResponse[] | { data: Project[] }>(
        `${API_URL}/portfolio/active`,
        {
          params: { limit }
        }
      );
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(
        ...
        (
          state,
          action: PayloadAction<FetchProjectsResponse>
        ) => {
          state.loading = false;
      ...
      .addCase(
        ...
        (state, action: PayloadAction<Project>) => {
          ...
          const activeIndex =
            state.activeProjects.findIndex(
              (project) =>
                project._id === action.payload._id
            );
          ...
          if (
            ...
            state.activeProjects[activeIndex] =
              action.payload;
          } else if (
            activeIndex !== -1 &&
            action.payload.status !== "active"
